...
# - IMPORTANT: We preserve any existing composite token during seeding and only rotate when FORCE_ROTATE=true.
# -------------------------------------------------------------------------------------------------
set -euo pipefail
...
PY
  ...
  while IFS=$'\t' read -r P DATA; do
    ...
    # Special handling for the config-server composite doc:
    # - do NOT clobber existing token during seeding
    # - use jq-only merge to avoid JSONDecodeError
    if [[ "$P" == "danipa-config-server,composite" ]]; then
      existing="$(vcurl "$VAULT_ADDR/v1/$MOUNT/data/danipa-config-server,composite" \
                 | jq -c '.data.data // {}' 2>/dev/null || echo '{}')"
      desired="$(jq -c 'del(."spring.cloud.config.server.composite[0].token")' <<<"$DATA" 2>/dev/null || echo '{}')"
      keep_token="$(jq -r '."spring.cloud.config.server.composite[0].token" // empty' <<<"$existing" 2>/dev/null || true)"

      # Merge desired over existing (seed wins), and keep token unless rotating
      merged="$(jq -c -s '.[0] * .[1]' <(printf '%s' "$existing") <(printf '%s' "$desired"))"
      if [[ -n "$keep_token" && "${FORCE_ROTATE:-false}" != "true" ]]; then
        merged="$(jq -c --arg t "$keep_token" '."spring.cloud.config.server.composite[0].token"=$t' <<<"$merged")"
      fi
...
PY
  ...
  # 2) Merge into danipa-config-server,composite in Vault using jq
  local existing merged
  existing="$(vcurl "$VAULT_ADDR/v1/$MOUNT/data/danipa-config-server,composite" \
             | jq -c '.data.data // {}' 2>/dev/null || echo '{}')"
  merged="$(jq -c --arg t "$tok" '. + {"spring.cloud.config.server.composite[0].token": $t}' <<<"$existing")"
...
for env in "${envs[@]}"; do
  ...
  # Only mint/overwrite token if missing or when FORCE_ROTATE=true
  existing_tok="$(vcurl "$VAULT_ADDR/v1/$MOUNT/data/danipa-config-server,composite" \
                   | jq -r '.data.data["spring.cloud.config.server.composite[0].token"] // empty' 2>/dev/null || true)"
  if [[ -n "$existing_tok" && "${FORCE_ROTATE:-false}" != "true" ]]; then
    echo "Composite token already present; skipping mint/write. (Use FORCE_ROTATE=true to rotate.)"
  else
...
path "secret/data/danipa/fintech/*"            { capabilities = ["read"] }
path "secret/metadata/danipa/fintech/*"        { capabilities = ["read","list"] }
HCL
)
