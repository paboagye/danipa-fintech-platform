...
SERVICE="fintech-service"                     # docker compose service name
CONTAINER="danipa-fintech-service"            # container_name
HOST_LOGDIR="logs/fintech"                    # host-side folder for captured logs

# Internal health: your fintech service runs on 8080
HEALTH_URL_INTERNAL="${HEALTH_URL_INTERNAL:-http://127.0.0.1:8080/actuator/health}"

# Optional external probe via Traefik/HTTPS
HEALTH_URL_EXTERNAL="${HEALTH_URL_EXTERNAL:-https://fintech.local.danipa.com/actuator/health}"
CA_CERT="${CA_CERT:-infra/vault/tls/root_ca.crt}"

# Optional gated health if you add it later
HEALTH_GATED_URL="${HEALTH_GATED_URL:-http://127.0.0.1:8080/actuator/health-gated}"
BEARER_TOKEN="${BEARER_TOKEN:-}"              # set to run gated check

# In-app rolling log inside the container
APPFILE="/app/logs/danipa-fintech-service/danipa-fintech-service.log"
...
docker exec "$CONTAINER" sh -lc 'rm -f /app/logs/danipa-fintech-service/* || true' || true
